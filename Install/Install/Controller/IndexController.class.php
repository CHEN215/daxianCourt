...
/**
	...
	public function setconf() {
		...
		if ($_POST) {
			...
			extract($_POST);
			if (!$db_type ||!$db_host || !$db_port || !$db_name || !$db_user || !$db_prefix || !$admin_user || !$admin_pass) {
			...
			$this->assign('db_name', 'zqcool');
			$this->assign('db_prefix', 'jl_');
...
/**
	...
	public function finish_done() {
		...
		foreach ($sqls as $sql) {
			//替换前缀
			$sql = str_replace('`jl_', '`' . $temp_info['db_prefix'], $sql);
			//获得表名
		...
		$sqls[] = "INSERT INTO `" . $temp_info['db_prefix'] . "user` VALUES " .
		      "(1, '". $temp_info['admin_user'] ."', '" . $password . "',".NOW_TIME.", '".get_client_ip(0)."', 0, '超级管理员', '', '')";
		...
		$config_data['AUTH_SUPERADMIN'] = $temp_info['admin_user'];
